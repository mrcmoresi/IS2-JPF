...
Otra herramienta desarrollada bajo el mismo evento es Symbolic Path Finder (SPF)[8] es una herramienta de ejecuci\'on simb\'olica basada en JPF. La misma ejecuta el JavaBytecode usando un JVM personalizada para realizar su an\'alisis actualmente se encuentra en desarrollo para agregarle ciertas funcionalidades.\\
Por \'ultimo nos parece imporante mencionar que dentro del mismo event\'o se desarroll\'o una extensi\'on de JPF muy interesante que es JPF Inspector[9], el cual permite ser agregado a un programa que est\'e usando la m\'aquina virtual de JPF para debugear el programa paso a paso, realizar ejecuciones de un solo paso entre otras funcionalidades que agrega.
...
\bibitem{ref_url1}
JPF Inspector - \url{https://github.com/d3sformal/jpf-inspector}
